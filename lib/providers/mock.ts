/**
 * Mock Provider - 기존 mockTranscribeSummarize 로직을 Provider 인터페이스로 래핑
 */
import type { MeetingNotesProvider, ProcessResult } from "./types";
...
const MOCK_SUMMARIES = [
  ...
  },
  {
    ...
    risks: ["API 레이턴시 이슈 미해결 시 모바일 앱 출시 일정 지연 가능"],
  },
  {
...
export class MockProvider implements MeetingNotesProvider {
  async process(
    _audioBuffer: Buffer,
    _mimeType: string,
    _durationSec: number
